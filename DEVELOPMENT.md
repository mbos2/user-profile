# 🛠️ Developing robo-plugin-user-profile
...
```bash
npx robo add /path/to/robo-plugin-user-profile
```

Replace `/path/to/robo-plugin-user-profile` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
